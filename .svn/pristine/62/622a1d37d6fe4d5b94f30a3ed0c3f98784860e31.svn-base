		...
		<script type="text/javascript">
			$(function(){
				$("#add_btn").click(function(){
					...
					var duty = window.prompt("工作职责");
					var demand = window.prompt("工作要求");
					$.ajax({
			            type: "POST",
			            url: "/recruit/addRecruit.do",
			            data:"recrJob="+job+"&recrPlace="+place+"&recrSalary="+salary+"&recrDegree="+degree+"&recrDuty="+duty+"&recrDemand="+demand,
				...
				$("#update_btn").click(function(){
					...
					$.ajax({
			            type: "POST",
			            url: "/recruit/updateRecruit.do",
			            data:"recrId="+id+"&recrJob="+job+"&recrPlace="+place+"&recrSalary="+salary+"&recrDegree="+degree+"&recrDuty="+duty+"&recrDemand="+demand,
			            success: function(data) {
			               if(data){
			               		console.log(data)
			               		alert(data)
				...
				$("#search_btn").click(function(){
					$.ajax({
			            type: "POST",
			            url: contextPath + "/recruit/findRecruits.do",
			            success: function(data) {
			               if(data){
			               		console.log(data)
			               		alert(data.length)
				...
				$("#searchAll_btn").click(function(){
					...
					var inputData3 = window.prompt("输入关键字");
					$.ajax({
			            type: "POST",
			            url: contextPath + "/recruit/findRecruitPages.do",
			            data:"pageIndex="+inputData1+"&pageSize="+inputData2+"&keywords="+inputData3,
			            success: function(data) {
			               if(data){
			               		console.log(data)
			               		alert(data.length)
